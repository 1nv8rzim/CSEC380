...
class WebCrawler:
    def __init__(self, domain, port=None, https=False, depth=1, threads=10, processes=2, path='', queue=None):
        self.domain = domain
        self.port = port
        self.https = https
        self.depth = depth
    ...
    def main(self):
        ...
        thread = self.WebCrawlerThread(self.domain, self.queue, self.all_urls,
                                       https=self.https, init=True, depth=self.depth)
        thread.start()
        thread.join()
            ...
            while not self.urls.empty():
                ...
                if '\r\n' in url:
                    ...
                    if len(temp) > 1:
                        ...
                        url = temp[0]
                request_type = 'https' if self.https else 'http'
                print(
